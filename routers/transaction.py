...
async def retrieve_by_ticker(params: tranactSchema.TransactionParams = Depends(), current_user: authSchema.Client = Security(get_current_client, scopes=["read"]), db: Session=Depends(get_db)):
...
async def retrieve_by_date_range(params: tranactSchema.TransactionDateRange = Depends(), current_user: authSchema.Client = Security(get_current_client, scopes=["read"]), db: Session=Depends(get_db)):
